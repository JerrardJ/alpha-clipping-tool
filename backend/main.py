...
logger = logging.getLogger("alpha-clips")

UPLOADS_DIR = TEMP_DIR / "uploads"
UPLOADS_DIR.mkdir(parents=True, exist_ok=True)

app = FastAPI(title="ALPHA CLIPS API", description="Alpha Clips Helps You More")
...
def health_check():
    ...
    return {
        "status": "ok",
        "message": "ALPHA CLIPS API is active",
    ...
    async def stream():
        ...
        if is_mock:
            ...
            result = AnalyzeResponse(
                ...
                summary="Mock analysis: this video explains how ALPHA CLIPS works. #aitools #videoediting #productivity",
                clips=mock_clips,
                model="Mock Gemini"
            )
            yield _sse({
...
async def process_batch_rendering(batch_id: str, request: RenderBatchRequest):
    ...
    try:
        zip_filename = f"alpha_clips_{batch_id}.zip"
...
def download_batch_zip(batch_id: str):
    ...
    safe_name = f"alpha_clips_{clean_id}.zip"
    file_path = EXPORTS_DIR / safe_name
    if not file_path.exists():
...
# Server reload trigger: rate-bypassed video engine active
