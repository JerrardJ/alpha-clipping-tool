...
logger = logging.getLogger("alpha-clips.video-engine")
...
def get_yt_dlp_base_cmd() -> List[str]:
    ...
    Always uses the active Python environment's module [sys.executable, "-m", "yt_dlp"]
    to ensure running the latest updated package with all challenge solvers.
    """
...
def download_clip_segment(
    ...
    """
    Downloads only the requested time slice in high definition (up to 1080p) using yt-dlp.
    Checks for local cached raw video first for instant slicing, then uses
    rate-bypassed HLS m3u8 streams to avoid YouTube 403 Forbidden throttles.
    """
    ...
    # Check if this video has already been downloaded locally (instant slicing)
    video_id_match = re.search(r'(?:v=|/v/|youtu\.be/|/embed/|/shorts/)([a-zA-Z0-9_-]{11})', clean_url)
    if video_id_match:
        vid_id = video_id_match.group(1)
        safe_id = re.sub(r'[^a-zA-Z0-9_-]', '_', vid_id)
        candidates = list(EXPORTS_DIR.glob(f"*{safe_id}*.mp4")) + list(TEMP_DIR.glob(f"*{safe_id}*.mp4"))
        for cand in candidates:
            if cand.is_file() and cand.stat().st_size > 500_000 and cand != output_path:
                logger.info(f"Found local raw video: {cand}. Trimming section directly via FFmpeg...")
                ...
                if output_path.exists() and output_path.stat().st_size > 10000:
                    logger.info(f"Direct local segment slice successful: {output_path} ({output_path.stat().st_size} bytes)")
    ...
    # Priority format selector:
    # 1. HLS m3u8 streams (ratebypass=yes, pre-chunked, completely avoids 403 Forbidden blocks)
    # 2. Standard AVC/H264 + AAC
    # 3. Best fallback
    format_selector = (
    ...
    # Method 1: yt-dlp --download-sections with rate-bypassed format selection
    cmd = [
        *base_cmd,
    ...
    # Method 2: Fallback - Extract direct stream URLs with yt-dlp and slice using FFmpeg with User-Agent
    ...
    try:
        ...
        if url_res.returncode == 0 and url_res.stdout.strip():
            ...
            headers_arg = f"User-Agent: {user_agent}\r\nAccept: */*\r\n"
            trim_cmd = [
                "ffmpeg", "-y", "-hide_banner", "-loglevel", "error",
                "-headers", headers_arg,
                "-ss", str(start_time),
